## list of keymaps
- win-g for window resizing
- Alt-hjkl for window snapping
- win-hl for desktop switching (l not working)
- Alt-q for closing
- Alt-f for fullscreen
- Alt-m for minimizing 
- Ctrl+Alt-T for alacritty 
- Ctrl+Alt-b for firefox
- Ctrl+Alt-b for gvim

<p>for these keymaps to work first u need to use autohotkey and make shortcuts for scipts and place them in _regedit_ **_shell:startup_** <p> 
note: ~~convert them into exe files by right clicking on them and compiling~~ need not to compile them its same as using ahk
- [ ] Ditch ahk and try to do the same using native windows
